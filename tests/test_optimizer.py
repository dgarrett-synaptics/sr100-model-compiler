...
def test_model_optimizer(
    ...
    success, results = sr100_model_optimizer(model_file=model_file)

    assert success is True, f'Optimization failed for {model_file}'
    assert results['model_loc'] == model_loc, f'{model_file} - Expected model location {model_loc}, got {results["model_loc"]}'
...
if __name__ == "__main__":
    ...
    for model_test in model_test_list:
        model_file_v, vmem_size_limit, lpmem_size_limit, model_loc_v = model_test
        test_model_optimizer(model_file_v, vmem_size_limit, lpmem_size_limit, model_loc_v)
