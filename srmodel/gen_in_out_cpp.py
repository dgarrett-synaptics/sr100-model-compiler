...
def main():
    ...
    parser.add_argument('-i', '--input', action='append', help="Input file(s) or wildcard pattern(s)", required=True)
    parser.add_argument('-e', '--output', action='append', help="Output file(s) or wildcard pattern(s)", required=False)
    parser.add_argument('-n', '--namespace', type=str, help="Namespace for the generated code", required=True)
    parser.add_argument('-o', '--output_dir', type=str, help="Directory to save the generated 'model_io.cc' file", required=True)
    ...
    rendered = template.render(namespace=args.namespace,
...
def read_file_data(file_path):
    ...
    with open(file_path, 'rb') as f:
        ...
        signed_chars = [(int.from_bytes(byte, byteorder='little', signed=True)) for byte in bytes_data]

        # Split the array into chunks of 32 bytes, formatting each byte as an integer
        # and joining with ', ' within each chunk and '\n' between chunks
        formatted_data = ',\n'.join(
            [', '.join([str(b) for b in chunk]) for chunk in np.array_split(signed_chars, len(signed_chars) / 32 + 1)]
        )
