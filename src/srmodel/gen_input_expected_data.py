...
def generate_input_expected_data(tflite_path, output_folder, namespace, license_header, input_files=None):
    # Load the model
    interpreter = tf.lite.Interpreter(model_path=tflite_path, experimental_preserve_all_tensors=True, experimental_op_resolver_type=tf.lite.experimental.OpResolverType.BUILTIN_REF)
    ...
    for i, input_detail in enumerate(input_details):
        ...
        if (input_files != None) and (i < len(input_files)):
            ...
            if file_extension == ".npy":
                ...
                with open(input_files[i], 'rb') as file:
                    ...
                    bin_data = file.read()
                    input_data = np.frombuffer(bin_data, dtype=np.int8).reshape(input_shape)
                    input_found = True
        ...
        input_data_str = ',\n'.join([', '.join([str(b) for b in a]) for a in np.array_split(input_data.flatten(), input_data.nbytes / 32 + 1)])
    ...
    for i, output_detail in enumerate(output_details):
        ...
        output_data_str = ',\n'.join([', '.join([str(b) for b in a]) for a in np.array_split(output_data.flatten(), output_data.nbytes / 32 + 1)])
    ...
    output = template.render(namespace=namespace, input_data_list=input_data_list, output_data_list=output_data_list, input_data_size_list=input_data_size_list, output_data_size_list=output_data_size_list)
    ...
    if platform.system() == "Windows":
        print(f"++ Generated input and expected output of {os.path.basename(tflite_path)} to {os.path.abspath(output_folder)}\{namespace}_io.cc")
    else:
        print(f"++ Generated input and expected output of {os.path.basename(tflite_path)} to {os.path.abspath(output_folder)}/{namespace}_io.cc")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run inference on a TensorFlow Lite model and generate input/output arrays in C++ format')
    ...
    parser.add_argument('-i', '--input', type=str, help='Folder containing input npy files with input_x.npy format')
    args = parser.parse_args()

    license_header = ""
    
    generate_input_expected_data(args.tflite_path, args.output_folder, args.namespace, license_header, args.input)
